...
if grep -Eq '^fast\.example[[:space:]]+4\.77 MiB/s[[:space:]]+selected
reset
...
exit "$(( FAIL > 0 ? 1 : 0 ))"
 <<<"$bench" &&
   grep -Eq '^slow\.example[[:space:]]+293 KiB/s' <<<"$bench" &&
   grep -Eq '^direct[[:space:]]+98 KiB/s' <<<"$bench"; then
  pass 'benchmark reports every route and marks the fastest sample'
else
  fail 'benchmark reports every route and marks the fastest sample'
fi
